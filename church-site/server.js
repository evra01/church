...
// Le front-end existant attend du camelCase (createdAt, paiementStatut...)
// alors que Postgres renvoie du snake_case. On convertit ici plutôt que de
// tout réécrire côté client.
...
const withCamelDate = camelizeRow;
const withCamelDates = camelizeRows;
...
const DEFAULT_SETTINGS = {
  ...
  paiementMesse: {
    actif: true,
    montant: 2000,
    instructions: "Le règlement se fait exclusivement via l'un des moyens ci-dessous, au nom de la paroisse. Une fois le paiement effectué, votre demande sera confirmée après vérification par le secrétariat.",
    wave: { actif: true, lien: '', numero: '', nom: '' },
  ...
  paiementDons: {
    actif: false,
    instructions: "Choisissez un moyen de paiement ci-dessous pour effectuer votre don, au nom de la paroisse. Votre don sera enregistré après vérification par le secrétariat.",
    wave: { actif: false, lien: '', numero: '', nom: '' },
    orangeMoney: { actif: false, numero: '', nom: '' },
    mtnMoney: { actif: false, numero: '', nom: '' },
    moovMoney: { actif: false, numero: '', nom: '' }
  },
...
async function initDB() {
  await pool.query(`
    ...
    ALTER TABLE messes ADD COLUMN IF NOT EXISTS montant NUMERIC;
    ALTER TABLE messes ADD COLUMN IF NOT EXISTS paiement_statut TEXT DEFAULT 'non_requis';
    ALTER TABLE messes ADD COLUMN IF NOT EXISTS paiement_methode TEXT;
    ALTER TABLE messes ADD COLUMN IF NOT EXISTS paiement_telephone TEXT;
    ALTER TABLE messes ADD COLUMN IF NOT EXISTS reference_paiement TEXT;
    CREATE TABLE IF NOT EXISTS dons (
      id TEXT PRIMARY KEY, nom TEXT, email TEXT, montant NUMERIC, frequence TEXT,
      created_at TIMESTAMPTZ DEFAULT now()
    );
    ALTER TABLE dons ADD COLUMN IF NOT EXISTS paiement_statut TEXT DEFAULT 'non_requis';
    ALTER TABLE dons ADD COLUMN IF NOT EXISTS paiement_methode TEXT;
    ALTER TABLE dons ADD COLUMN IF NOT EXISTS paiement_telephone TEXT;
    ALTER TABLE dons ADD COLUMN IF NOT EXISTS reference_paiement TEXT;
...
app.put('/api/admin/settings', requireAuth, async (req, res) => {
  ...
  if (!s.paiementMesse) {
    s.paiementMesse = {
      actif: true, montant: 0, instructions: '',
      wave: { actif: false, lien: '', numero: '', nom: '' },
      orangeMoney: { actif: false, numero: '', nom: '' },
      mtnMoney: { actif: false, numero: '', nom: '' },
      moovMoney: { actif: false, numero: '', nom: '' }
  ...
  if (body.paiementMesse && typeof body.paiementMesse === 'object') {
    const pm = body.paiementMesse;
    if (typeof pm.actif === 'boolean') s.paiementMesse.actif = pm.actif;
    if (pm.montant !== undefined && Number.isFinite(Number(pm.montant)) && Number(pm.montant) >= 0) s.paiementMesse.montant = Number(pm.montant);
    if (typeof pm.instructions === 'string') s.paiementMesse.instructions = pm.instructions;
    for (const cle of ['wave', 'orangeMoney', 'mtnMoney', 'moovMoney']) {
      if (!pm[cle] || typeof pm[cle] !== 'object') continue;
      if (!s.paiementMesse[cle]) s.paiementMesse[cle] = { actif: false, numero: '', nom: '' };
      const dest = s.paiementMesse[cle];
      const src = pm[cle];
      if (typeof src.actif === 'boolean') dest.actif = src.actif;
      if (typeof src.numero === 'string') dest.numero = src.numero;
      if (typeof src.nom === 'string') dest.nom = src.nom;
      if (cle === 'wave' && typeof src.lien === 'string') dest.lien = src.lien;
    }
  }
  if (!s.paiementDons) {
    s.paiementDons = {
      actif: false, instructions: '',
      wave: { actif: false, lien: '', numero: '', nom: '' },
      orangeMoney: { actif: false, numero: '', nom: '' },
      mtnMoney: { actif: false, numero: '', nom: '' },
      moovMoney: { actif: false, numero: '', nom: '' }
  ...
  if (body.paiementDons && typeof body.paiementDons === 'object') {
    const pd = body.paiementDons;
    if (typeof pd.actif === 'boolean') s.paiementDons.actif = pd.actif;
    if (typeof pd.instructions === 'string') s.paiementDons.instructions = pd.instructions;
    for (const cle of ['wave', 'orangeMoney', 'mtnMoney', 'moovMoney']) {
      if (!pd[cle] || typeof pd[cle] !== 'object') continue;
      if (!s.paiementDons[cle]) s.paiementDons[cle] = { actif: false, numero: '', nom: '' };
      const dest = s.paiementDons[cle];
      const src = pd[cle];
      if (typeof src.actif === 'boolean') dest.actif = src.actif;
      if (typeof src.numero === 'string') dest.numero = src.numero;
      if (typeof src.nom === 'string') dest.nom = src.nom;
...
app.post('/api/messes', async (req, res) => {
  const { nom, email, date, type, note, methodePaiement, telephonePaiement, referencePaiement } = req.body || {};
  if (!nom || !email || !date) return res.status(400).json({ error: 'Nom, e-mail et date sont requis' });

  // Le montant et la validité du moyen de paiement sont déterminés côté
  // serveur à partir des réglages actuels, jamais à partir de ce qu'envoie
  ...
  const pm = (settingsRows[0].data && settingsRows[0].data.paiementMesse) || { actif: false, montant: 0 };

  let montant = null;
  let paiementStatut = 'non_requis';
  let methode = null;
  let telephone = null;
  let reference = null;

  if (pm.actif) {
    const methodesValides = ['wave', 'orangeMoney', 'mtnMoney', 'moovMoney'];
    const methodeInfo = methodesValides.includes(methodePaiement) ? pm[methodePaiement] : null;
    if (!methodeInfo || !methodeInfo.actif) {
      return res.status(400).json({ error: "Le moyen de paiement choisi n'est pas disponible" });
    }
    if (methodePaiement === 'wave') {
      if (!telephonePaiement || String(telephonePaiement).replace(/\D/g, '').length < 8) {
        return res.status(400).json({ error: 'Merci d\'indiquer le numéro utilisé pour payer avec Wave' });
      }
      telephone = String(telephonePaiement).slice(0, 30);
    } else {
      if (!referencePaiement || !String(referencePaiement).trim()) {
        return res.status(400).json({ error: 'Merci d\'indiquer la référence de la transaction' });
      }
      reference = String(referencePaiement).slice(0, 100);
    }
    montant = Number(pm.montant) || 0;
  ...
  const { rows } = await pool.query(
    `INSERT INTO messes (id, nom, email, date, type, note, status, montant, paiement_statut, paiement_methode, paiement_telephone, reference_paiement)
     VALUES ($1,$2,$3,$4,$5,$6,'en_attente',$7,$8,$9,$10,$11) RETURNING *`,
    [id, nom, email, date, type || 'Autre intention', note || '', montant, paiementStatut, methode, telephone, reference]
  );
...
app.patch('/api/admin/messes/:id/paiement', requireAuth, async (req, res) => {
  const { paiementStatut } = req.body || {};
  const allowed = ['non_paye', 'paye', 'non_requis'];
  if (!allowed.includes(paiementStatut)) return res.status(400).json({ error: 'Statut de paiement invalide' });
  const { rows } = await pool.query('UPDATE messes SET paiement_statut = $1 WHERE id = $2 RETURNING *', [paiementStatut, req.params.id]);
  if (!rows.length) return res.status(404).json({ error: 'Demande introuvable' });
  res.json(withCamelDate(rows[0]));
});
...
app.post('/api/dons', async (req, res) => {
  const { nom, email, montant, frequence, methodePaiement, telephonePaiement, referencePaiement } = req.body || {};
  ...
  // La validité du moyen de paiement est déterminée côté serveur à partir
  // des réglages actuels, jamais à partir de ce qu'envoie le client.
  const { rows: settingsRows } = await pool.query('SELECT data FROM app_settings WHERE id = 1');
  const pd = (settingsRows[0].data && settingsRows[0].data.paiementDons) || { actif: false };
  ...
  if (pd.actif) {
    const methodesValides = ['wave', 'orangeMoney', 'mtnMoney', 'moovMoney'];
    const methodeInfo = methodesValides.includes(methodePaiement) ? pd[methodePaiement] : null;
    if (!methodeInfo || !methodeInfo.actif) {
      return res.status(400).json({ error: "Le moyen de paiement choisi n'est pas disponible" });
    }
    if (methodePaiement === 'wave') {
  ...
  const id = uid('d');
  const { rows } = await pool.query(
    `INSERT INTO dons (id, nom, email, montant, frequence, paiement_statut, paiement_methode, paiement_telephone, reference_paiement)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
    [id, nom, email, Number(montant), freqValide, paiementStatut, methode, telephone, reference]
  );
  // NOTE : le paiement est déclaré par le donateur puis vérifié manuellement
  // par le secrétariat (voir /api/admin/dons/:id/paiement). Pour un
  // encaissement automatique, il faut intégrer un prestataire tiers.
  res.status(201).json(withCamelDate(rows[0]));
});
...
app.patch('/api/admin/dons/:id/paiement', requireAuth, async (req, res) => {
  const { paiementStatut } = req.body || {};
  const allowed = ['non_paye', 'paye', 'non_requis'];
  if (!allowed.includes(paiementStatut)) return res.status(400).json({ error: 'Statut de paiement invalide' });
  const { rows } = await pool.query('UPDATE dons SET paiement_statut = $1 WHERE id = $2 RETURNING *', [paiementStatut, req.params.id]);
  if (!rows.length) return res.status(404).json({ error: 'Don introuvable' });
  res.json(withCamelDate(rows[0]));
});
